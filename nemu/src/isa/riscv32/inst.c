...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc = ((unsigned)src1 >= (unsigned)src2) ? (SEXT(imm, 13) + s->pc) : s->dnpc);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (src1 >= src2) ? (SEXT(imm, 13) + s->pc) : s->dnpc);
