...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (signed)src1 / (signed)src2);  //M
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src1 / src2);  //M
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (signed)src1 % (signed)src2);  //M
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = csr_read(BITS(s->isa.inst, 31, 20)), csr_write(BITS(s->isa.inst, 31, 20), src1)); if(BITS(s->isa.inst, 31, 20) == 0x341) printf("0x341\n"); 
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t old = csr_read(BITS(s->isa.inst, 31, 20)); R(rd) = old; csr_write(BITS(s->isa.inst, 31, 20), src1 | old)); if(BITS(s->isa.inst, 31, 20) == 0x341) printf("0x341\n");

  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10)); /*printf("R(10): %d\n", R(10));*/);  // R(10) is $a0 
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(11, s->pc)); printf("0x341\n");
