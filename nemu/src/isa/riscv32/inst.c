...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = imm + src1, R(rd) = s->snpc, ftrace_exec(s->pc, s->dnpc, R(rd), src1, imm));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = SEXT(imm, 21) + s->pc, R(rd) = s->snpc, ftrace_exec(s->pc, s->dnpc, R(rd), 65, imm));
